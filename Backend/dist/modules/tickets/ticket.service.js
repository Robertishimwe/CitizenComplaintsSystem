...
const ticket_ai_service_1 = require("./services/ticket.ai.service");
const prisma_1 = __importDefault(require("@/config/prisma"));
const client_1 = require("@prisma/client"); // Added Communication
const config_1 = require("@/config");
class TicketService {
    constructor() {
        this.ticketRepository = new ticket_repository_1.TicketRepository();
        this.notificationService = new notification_service_1.NotificationService();
        this.ticketAiService = new ticket_ai_service_1.TicketAiService();
    }
    // Regular private method
    mapCommunicationToResponse(comm) {
        const response = {
            id: comm.id,
    ...
    async createTicket(dto, creatingUser) {
        ...
        payload.status = client_1.TicketStatus.NEW; // Default status
        // payload.assignedAgencyId will be set if routing occurs
        let categoryObject = null;
        let categoryNameForAi = undefined;
        if (creatingUser) {
            payload.isAnonymous = false;
            payload.citizenId = creatingUser.id;
        }
            ...
            if (!dto.anonymousCreatorName || !dto.anonymousCreatorContact) {
                throw new ApiError_1.ApiError(400, 'Name and contact are required for non-logged-in submissions not marked anonymous.');
            }
        ...
        // 1. Handle Category and Attempt Rule-Based Assignment
        if (dto.categoryId) {
            categoryObject = await prisma_1.default.category.findUnique({ where: { id: dto.categoryId } });
            console.log("<><><><><>", categoryObject);
            if (!categoryObject) {
                throw new ApiError_1.ApiError(400, `Invalid category ID: "${dto.categoryId}" not found.`);
            }
            payload.categoryId = dto.categoryId; // Ensure it's in payload
            categoryNameForAi = categoryObject.name;
            const routingRule = await prisma_1.default.routingRule.findUnique({
                where: { categoryId: dto.categoryId, status: client_1.ResourceStatus.ACTIVE },
            });
            if (routingRule && routingRule.assignedAgencyId) {
                const agency = await prisma_1.default.agency.findUnique({ where: { id: routingRule.assignedAgencyId } });
                if (agency && agency.status === client_1.ResourceStatus.ACTIVE) {
                    payload.assignedAgencyId = routingRule.assignedAgencyId; // Set on payload
                    payload.status = client_1.TicketStatus.ASSIGNED; // Set on payload
                    config_1.logger.info(`Ticket auto-assigned to agency "${agency.name}" via rule. Status: ${payload.status}.`);
                }
                else {
                    config_1.logger.warn(`Rule-based: Agency for category "${categoryObject.name}" is inactive/not found. Will attempt AI.`);
                }
            }
            else {
                config_1.logger.info(`No active routing rule for category "${categoryObject.name}". Will attempt AI.`);
            }
        }
        else {
            config_1.logger.info('No category ID provided by user. Will attempt AI.');
        }
        ...
        if (!payload.assignedAgencyId) { // Check if agency was assigned in the step above
            config_1.logger.info(`Attempting AI-based agency suggestion. Category for AI context: "${categoryNameForAi || 'None'}".`);
            const allCategories = await prisma_1.default.category.findMany({ select: { id: true, name: true, description: true } });
            const allAgencies = await prisma_1.default.agency.findMany({ where: { status: client_1.ResourceStatus.ACTIVE }, select: { id: true, name: true, description: true } });
            const allRoutingRulesFromDb = await prisma_1.default.routingRule.findMany({
                where: { status: client_1.ResourceStatus.ACTIVE },
            ...
            const aiSuggestedAgencyId = await this.ticketAiService.suggestAgencyForTicket({ title: dto.title, detailedDescription: dto.detailedDescription, categoryName: categoryNameForAi }, aiContext);
            if (aiSuggestedAgencyId) {
                ...
                if (aiAgency) {
                    ...
                    payload.status = client_1.TicketStatus.ASSIGNED; // Set on payload
                    config_1.logger.info(`Ticket auto-assigned to agency "${aiAgency.name}" via AI. Status: ${payload.status}.`);
                }
                else {
                    config_1.logger.warn(`AI suggested agency ID "${aiSuggestedAgencyId}" (not valid/active). Ticket remains unassigned by AI.`);
                }
            }
            else {
                config_1.logger.info('AI did not provide agency suggestion. Ticket remains unassigned by AI.');
            }
        }
    ...
    async getTicketById(ticketId, requestingUser) {
        ...
        if (!requestingUser) {
            throw new ApiError_1.ApiError(401, 'Authentication required to view this ticket.');
        }
        if (requestingUser.role === client_1.UserRole.ADMIN) {
            return this.mapTicketToResponse(ticket);
        }
        if (requestingUser.role === client_1.UserRole.CITIZEN && ticket.citizenId === requestingUser.id) {
            return this.mapTicketToResponse(ticket);
        }
        if (requestingUser.role === client_1.UserRole.AGENCY_STAFF && ticket.assignedAgencyId === requestingUser.agencyId) {
            return this.mapTicketToResponse(ticket);
        }
        throw new ApiError_1.ApiError(403, 'You are not authorized to view this ticket.');
    }
    async listTickets(query, requestingUser) {
    ...
    async updateTicketByAgent(ticketId, dto, agent) {
        const ticket = await this.ticketRepository.findById(ticketId);
        if (!ticket) {
            throw new ApiError_1.ApiError(404, 'Ticket not found.');
        }
        if (agent.role !== client_1.UserRole.ADMIN && ticket.assignedAgencyId !== agent.agencyId) {
            throw new ApiError_1.ApiError(403, 'You are not authorized to update this ticket.');
        }
        const payload = { ...dto };
    ...
    async updateTicketAssignment(ticketId, dto, adminUser) {
        const ticket = await this.ticketRepository.findById(ticketId);
        if (!ticket)
            throw new ApiError_1.ApiError(404, 'Ticket not found.');
        const payload = {};
        ...
        if (dto.assignedAgentId !== undefined) {
            if (dto.assignedAgentId) {
                const agentToAssign = await prisma_1.default.user.findUnique({ where: { id: dto.assignedAgentId } });
                if (!agentToAssign || agentToAssign.role !== client_1.UserRole.AGENCY_STAFF) {
                    throw new ApiError_1.ApiError(400, 'Invalid agent ID or user is not an agency staff.');
                }
                if (ticket.assignedAgencyId && agentToAssign.agencyId !== ticket.assignedAgencyId) {
                    throw new ApiError_1.ApiError(400, 'Agent does not belong to the ticket\'s currently assigned agency.');
                }
                ...
                newStatus = (ticket.status === client_1.TicketStatus.NEW || ticket.status === client_1.TicketStatus.IN_PROGRESS_PENDING_AGENT) ? client_1.TicketStatus.ASSIGNED : ticket.status;
            }
            else {
                payload.assignedAgentId = null;
                newStatus = client_1.TicketStatus.IN_PROGRESS_PENDING_AGENT;
            }
        }
    ...
    async addCommunication(ticketId, dto, sendingUser) {
        const ticket = await this.ticketRepository.findById(ticketId);
        if (!ticket) {
            throw new ApiError_1.ApiError(404, 'Ticket not found.');
        }
        ...
        if (sendingUser) {
            if (sendingUser.role === client_1.UserRole.ADMIN)
                canComment = true;
            else if (sendingUser.role === client_1.UserRole.CITIZEN && ticket.citizenId === sendingUser.id && !ticket.isAnonymous)
                canComment = true;
            else if (sendingUser.role === client_1.UserRole.AGENCY_STAFF && ticket.assignedAgencyId === sendingUser.agencyId)
                canComment = true;
        }
        if (!canComment) {
            throw new ApiError_1.ApiError(403, 'You are not authorized to add communication to this ticket.');
        }
        if (dto.isInternal && sendingUser?.role === client_1.UserRole.CITIZEN) {
            throw new ApiError_1.ApiError(400, 'Citizens cannot add internal notes.');
        }
        const payload = { ...dto, ticketId };
        if (sendingUser) {
        ...
        if (sendingUser?.role === client_1.UserRole.CITIZEN && ticket.status === client_1.TicketStatus.IN_PROGRESS_PENDING_CITIZEN) {
            await this.ticketRepository.update(ticketId, { status: client_1.TicketStatus.IN_PROGRESS_PENDING_AGENT });
            statusChanged = true;
        }
        // Notify relevant parties for public messages
        if (!dto.isInternal) {
            if (sendingUser?.role === client_1.UserRole.AGENCY_STAFF && ticket.citizen?.phone) {
    ...
    async transferTicket(ticketId, dto, transferringAgent) {
        const ticket = await this.ticketRepository.findById(ticketId);
        if (!ticket)
            throw new ApiError_1.ApiError(404, 'Ticket not found.');
        if (transferringAgent.role !== client_1.UserRole.AGENCY_STAFF && transferringAgent.role !== client_1.UserRole.ADMIN) {
            throw new ApiError_1.ApiError(403, 'Only agency staff or admins can transfer tickets.');
        }
        if (transferringAgent.role === client_1.UserRole.AGENCY_STAFF && ticket.assignedAgencyId !== transferringAgent.agencyId) {
            throw new ApiError_1.ApiError(403, 'You can only transfer tickets assigned to your agency.');
        }
        if (ticket.assignedAgencyId === dto.newAgencyId) {
            throw new ApiError_1.ApiError(400, 'Ticket is already assigned to this agency.');
        }
        const newAgency = await prisma_1.default.agency.findUnique({ where: { id: dto.newAgencyId } });
        if (!newAgency)
            throw new ApiError_1.ApiError(404, 'Target agency for transfer not found.');
        if (newAgency.status === client_1.ResourceStatus.INACTIVE)
            throw new ApiError_1.ApiError(400, 'Cannot transfer ticket to an inactive agency.');
        const updatePayload = {
            assignedAgencyId: dto.newAgencyId,
            assignedAgentId: null,
            status: client_1.TicketStatus.IN_PROGRESS_PENDING_AGENT,
        };
