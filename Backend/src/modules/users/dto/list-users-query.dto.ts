...
const optionalNumericTransform = () => {
  return z.string()
    .optional() // Input string is optional
    .refine(val => { // Refine needs to handle undefined if input is optional before transform
        if (val === undefined) return true; // Allow undefined to pass if optional
...
export const ListUsersQuerySchema = z.object({
  query: z.object({
    ...
    search: z.string().min(1).optional(), // If search can be empty string vs not present, adjust
