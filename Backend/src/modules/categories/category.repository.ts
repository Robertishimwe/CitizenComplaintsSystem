...
import { Prisma, Category } from '@prisma/client'; // Category is the base type
import { CreateCategoryPayload, UpdateCategoryPayload, CategoryFilter } from './interfaces/category.interfaces';

// Define a type for Category with its relations as fetched by findById and update
...
// Define a type for Category as fetched in a list (findAll)
type CategoryInList = Prisma.CategoryGetPayload<{
  include: {
    parentCategory: { select: { id: true; name: true } };
    // subCategories could be included here if needed for lists
  };
}>;

// Type for items in the recursive tree
type CategoryTreeNode = Prisma.CategoryGetPayload<{
  select: {
    id: true;
    name: true;
    description: true;
...
export class CategoryRepository {
  async create(data: CreateCategoryPayload): Promise<CategoryWithDetails> {
  ...
  async findById(id: string): Promise<CategoryWithDetails | null> { // CORRECTED RETURN TYPE
  ...
  ): Promise<{ categories: CategoryInList[]; total: number }> {
  ...
  async update(id: string, data: UpdateCategoryPayload): Promise<CategoryWithDetails | null> { // CORRECTED RETURN TYPE
  ...
  async delete(id: string): Promise<Category | null> { // Returns base Category or null
    ...
    if (categoryWithDetails.subCategories && categoryWithDetails.subCategories.length > 0) {
      console.warn(
        `Deleting category "${categoryWithDetails.name}" which has ${categoryWithDetails.subCategories.length} sub-categories. Their parent will be set to null due to schema's onDelete behavior.`
      );
    }
  ...
  async getCategoryTree(parentId: string | null = null): Promise<CategoryTreeNode[]> {
    const categories = await prisma.category.findMany({
